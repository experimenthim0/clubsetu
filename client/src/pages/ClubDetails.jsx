...
const ClubDetails = () => {
  ...
  if (loading)
    return <div className="text-center py-20">Loading club details...</div>;
  if (!club)
    return (
      <div className="text-center py-20 text-red-600">Club not found.</div>
    );
  ...
  const EventCard = ({ event, type, wide = false }) => (
    <Link
      ...
      className={`block group bg-white border-2 border-gray-300  hover:translate-x-[-2px] hover:translate-y-[-2px]  transition-all p-5
                ${type === "live" ? "border-orange-600 " : ""}
    ...
    >
      <div className={wide ? "flex-1" : ""}>
        <div className="flex justify-between items-start mb-3">
          <span
            className={`text-[9px] font-black uppercase px-2 py-0.5 border rounded-sm ${getBadgeClass(type)}`}
          >
            {type === "live"
              ? "Live"
              : type === "upcoming"
        ...
        <h3
          className={`font-black text-base text-black group-hover:text-orange-600 transition-colors leading-tight mb-2 ${wide ? "text-xl" : ""}`}
        >
          {event.title}
        </h3>
      ...
      <div
        className={`flex items-center justify-between text-[9px] font-black uppercase tracking-wider ${wide ? "flex-col items-end gap-3 flex-shrink-0" : "mt-4 pt-3 border-t border-gray-100"}`}
      >
        <span className="text-neutral-400">{event.venue}</span>
        <span className="text-orange-600 hover:underline">
          {type === "past" ? "View Recap →" : "Register Now →"}
        </span>
      </div>
    </Link>
  );
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="bg-white text-black pt-20 pb-28 px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-10 relative z-10">
          <div className="w-32 h-32 bg-white border-[3px] border-gray-300  rounded-full flex-shrink-0">
            {club.clubLogo ? (
              <img
                src={club.clubLogo}
                alt={club.clubName}
          ...
          <div className="flex-1 text-center md:text-left">
            <h1 className="text-4xl md:text-5xl font-black tracking-wide mb-3 leading-none">
              {club.clubName}
            </h1>
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-12 relative z-20 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white text-black border-2 border-gray-300 p-5 rounded-sm">
            <p className="text-[12px] font-black tracking-widest text-neutral-500 mb-1">
              Faculty Coordinator
            </p>
          ...
          <div className="col-span-2 bg-white border-2 border-gray-300 p-5 rounded-sm ">
            <p className="text-[9px] font-black  tracking-widest text-neutral-400 mb-3">
              Connect with us
            </p>
              ...
              {club.socialLinks?.map((link, i) => {
                ...
                return (
                  <a
                    ...
                    className="w-10 h-10 flex items-center justify-center bg-white transition-all hover:translate-y-[2px]"
                    title={link.platform}
                  >
                    {getIcon()}
                  </a>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div
              key={label}
              className={`border-2 border-gray-300 p-4 rounded-sm  ${accent ? "bg-orange-600" : "bg-white"}`}
            >
              <div
                className={`text-3xl font-black leading-none ${accent ? "text-white" : "text-orange-600"}`}
              >
                {num}
              </div>
              <div
                className={`text-[9px] font-black uppercase tracking-widest mt-1 ${accent ? "text-orange-100" : "text-neutral-400"}`}
              >
                {label}
              </div>
            </div>
        ...
        {liveEvents.length > 0 && (
          <section>
            <div className="flex items-center gap-4 my-6">
              <h2 className="text-2xl font-black italic tracking-tighter whitespace-nowrap">
                Live Events
              </h2>
        ...
        <section>
          ...
          {club.clubGallery.length > 0 ? (
            <div className="grid grid-cols-3 gap-4">
              {club.clubGallery.map((image, index) => (
                <img
                  ...
                  src={image}
                  alt={`Gallery ${index + 1}`}
                  className="w-full h-full object-cover rounded-xs  hover:scale-105 transition-all duration-300"
                />
              ))}
            </div>
          ) : (
            <div className="bg-white border-2 border-dashed border-neutral-200 py-8 text-center">
              <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400">
                No image found, Lagta hai Clubhead ji add krna bhul gye.
              </p>
            </div>
          )}
            ...
            <div className="grid grid-cols-3 gap-4">
              {club.clubSponsors.map((image, index) => (
                <img
                  ...
                  src={image}
                  alt={`Sponsor ${index + 1}`}
                  className="w-full h-40 object-contain  rounded-xs  hover:scale-105 transition-all duration-300"
                />
              ))}
            </div>
          </section>
