...
const AdminDashboard = () => {
    ...
    const handleFetchPayoutInfo = async (clubHeadId, eventId) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/admin/club-head/${clubHeadId}`);
    ...
    const handleConfirmPayout = async () => {
        try {
            const res = await axios.post(`http://localhost:5000/api/admin/complete-payout/${selectedEventId}`);
            if (res.data.success) {
